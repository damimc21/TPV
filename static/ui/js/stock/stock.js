document.addEventListener('DOMContentLoaded', function() {
    ...
    window.selectCat = (id, nombre) => {
        const input = document.getElementById('filterCat');
        const display = document.getElementById('selectedCatName');
        input.value = id;
        ...
        document.getElementById('containerCat').classList.remove('is-active');
        renderView();
    };

    window.selectStatus = (id, nombre) => {
        const input = document.getElementById('filterStatus');
        const display = document.getElementById('selectedStatusName');
        input.value = id;
        display.innerText = nombre;

        const options = document.querySelectorAll('#containerStatus .custom-option');
        options.forEach(opt => {
            opt.classList.toggle('is-selected', opt.innerText === nombre);
        });

        document.getElementById('containerStatus').classList.remove('is-active');
        renderView();
    };
    ...
    function renderStockGrid() {
        ...
        filtered.forEach(a => {
            ...
            card.innerHTML = `
                ...
                <div class="card-auto-slot">${autoHtml}</div>
    ...
    function renderStockTable() {
        ...
        filtered.forEach(a => {
            ...
            tr.innerHTML = `
                <td class="td-product">${a.nombre}</td>
                <td><span class="card-dept card-dept--table">${catName}</span></td>
                <td>
                    <span class="stock-badge badge-${state}">
                        <span class="badge-dot"></span>
                        ${getStateLabel(state)}
                ...
                <td class="td-stock ${valClass}">${stockDisplay} <span class="text-xs text-slate-500 font-normal ml-1">${a.unidad}</span></td>
                <td class="td-min">${minDisplay}</td>
                <td class="td-auto">${autoLabel}</td>
                <td>
